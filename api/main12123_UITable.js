...
async function main() {
  ...
  async function getVerifyToken() {  
    ...
    if (!verifyToken) {
      ...
      await saveSettings();
      notify('12123_Referer‼️', '点击查机动车违法再点击查询即可更新/获取');
  ...
  'aHR0cDovL210dy5zby81Ulp4emc=', 'aHR0cDovL210dy5zby82dEFCQmU=']
    ...
    const updateVersion = [
      ...
      },
      {
        ...
        type: 'ver',
        title: '当前版本',
        desc: '2023年01月30日\n\n版本更新，如关闭自动更新的手动更新代码，以及自行更新Quantumult-X 重写或其他辅助工具\n\n⚠️ 注: 12123_Referer用于获取检验有效期的日期和累积记分以及车牌号需正确填写【 有违章时获取数据 】\n\n1，修复车辆违章后数据无法显示问题\n2，支持多车辆、多次违章( 随机显示 )\n3，随机显示违章照片，点击地址跳转\n4，增加显示检验有效期止日期  \n5，增加驾驶证累积记分',
        val: '1.1.0',
        ver: 'Version 1.1.0'
      },
      {
