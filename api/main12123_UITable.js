...
async function main() {
  ...
  async function renderTables(table) {
    ...
    const topImg = ['aHR0cDovL210dy5zby81UkoyQlM=', 'aHR0cDovL210dy5zby82Nkw0Q0U=', 
  'aHR0cDovL210dy5zby82ZjFVZjQ=']
      ...
      assist.forEach ((item) => {
        ...
        : async () => {
          if (type === 'input') {
            let set = new Alert();
            set.title = title;
            set.message = val !== 'gradient' ? desc : desc + setting[val]
            set.addTextField(
  ...
  async function updateVersion(title, desc) {
    ...
    if (index === 0) return;
    const reqUpdate = new Request(atob('aHR0cHM6Ly9naXRjb2RlLm5ldC80cWlhby9mcmFtZXdvcmsvcmF3L21hc3Rlci9hcGkvbWFpbjEyMTIzX1VJVGFibGUuanM='));
