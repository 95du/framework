  ...
  const renderAppView = async (options) => {
    ...
    const buttonPopup = async () => {
      ...
      const typeNextChar = () => {
        ...
        chatMsg.textContent = "";
        let currentChar = 0;
        const message = \`${settings.oilsAlert}\`
      ...
      const content = `${avatarInfo  
        ? `<img id="app" onclick="switchDrawerMenu()" class="app-icon" src="${appImage}">
          <div class="app-desc">全国柴油、汽油价格</div>
          <button class="but jb-green" id="adjustment" onclick="hidePopup()">2023 年油价调整日期</button>`  
    ...
    previewImgHtml = async () => {
      const previewImgUrl = [
        `${rootUrl}img/picture/oils_price_0.png`,
        `${rootUrl}img/picture/oils_price_1.png`
      ];
