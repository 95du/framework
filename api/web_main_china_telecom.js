...
async function main() {
  ...
  const renderAppView = async (options) => {
      ...
      <div class="modal fade" id="u_sign" role="dialog">
        <div class="modal-dialog">
          <div class="zib-widget blur-bg relative">
            ...
            <div class="box-body sign-logo" data-dismiss="modal" onclick="hidePopup()"><img src="${appleHub}">
            </div>
    ...
    // 登录中国电信获取 Cookie
    const webLogin = async (url) => {
      const webview = new WebView();
      await webview.loadURL(url);
      await webview.present();
      const cookie = await webview.evaluateJavaScript('document.cookie');
      settings.cookie = cookie.match(/(CZSSON=[\w\d]+)/)[0];
      if (!cookie.includes('configurable')) {
        notify('Cookie获取/更新成功', settings.cookie);
        writeSettings(settings);
        innerTextElementById('login', cookie ? '已登录' : '未登录');
      }
    };
    ...
    const injectListener = async () => {
      ...
      switch (code) {
        ...
        case 'updateCode':
          ...
          break;
        case 'login':
          await webLogin('http://u3v.cn/5uwtIP');
          break;
        case 'cookie':
          await webLogin('https://e.189.cn/store/wap/partner/stylehead/189Bill.do');
          break;
        case 'period':
          await period(data);
          break;
        ...
        items: [
          {
            ...
            type: 'cell',
            isAdd: true,
            icon: `${rootUrl}img/icon/telecom_0.png`,
            message: '输入token',
