...
async function main() {
  ...
  const path = F_MGR.joinPath(F_MGR.documentsDirectory(), "95duJingDong_Treasury");
  ...
  async function downloadModule() {
    ...
    if (setting.update === 'false' && F_MGR.fileExists(modulePath)) {
      ...
      const req = new Request('https://gitcode.net/4qiao/scriptable/raw/master/table/jingDong_Treasury.js');
  ...
  async function updateVersion(title, desc) {
    ...
    if (index === 0) return;
    const modulePath = F_MGR.joinPath(path, 'jingDong.js');
    const reqUpdate = new Request('https://gitcode.net/4qiao/scriptable/raw/master/table/jingDong_Treasury.js');
