...
async function main() {
  ...
  const version = '1.0.0'
  const updateDate = '2023年10月17日'
  
  const pathName = '95du_powerGrid';
  const widgetMessage = '组件功能: 1，当月用电量、账单。2，昨日用电量、余额。3，上月用电量、账单。4，当月欠费通知（ 未缴费时 ⚠️，每12小时通知一次 ）';
  
  const rootUrl = atob('aHR0cHM6Ly9naXRjb2RlLm5ldC80cWlhby9mcmFtZXdvcmsvcmF3L21hc3Rlci8=');

  const [scrName, scrUrl] = ['powerGrid.js', 'https://gitcode.net/4qiao/scriptable/raw/master/table/web_powerGrid.js'];
  ...
  const DEFAULT = {
    ...
    titleColor: '#000000',
    rangeColor: '#3F8BFF',
    progressWidth: screenSize < 926 ? '225' : '255',
  ...
  const renderAppView = async (options) => {
    ...
    const userlogin = async ({ label, name } = data) => {
      ...
      if ( openAlert === 1 ) {
        ...
        async ([{ value }]) => {
          result = /^[a-z\d-]+$/.test(value) ? value : '';
        });
        
        if (input === 1 && result) {
          settings.token = result;
          settings.updateTime = Date.parse(new Date());
          settings.code = 0
          notify('登录成功', result);
          writeSettings(settings);
